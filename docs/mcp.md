...
## 1. 为什么先是一个独立进程，而不是先内嵌进宿主页面

引擎也能内嵌进宿主页面（`wasm32` 壳，见 [`core.md`](./core.md)），但先做**独立进程**：

- 内嵌会绑在**一个**宿主的具体机制上（编译期 feature、页面上下文是否与 Agent 循环相同、宿主能不能下发 `COOP`/`COEP`……），
  这些问题的答案都在宿主那一侧，**每一条都能把开工卡住**。
- 引擎的价值在 `core`（观察 / 编辑 / 回滚 / 可复现），**与宿主形态无关**。先用通用接口把它接出来，价值立刻可见，
  而且反过来给宿主集成提供参照物。
- MCP 覆盖面最大：任何支持 MCP 的工具都能接，不押死在某一个宿主上。

所以顺序是：**先 MCP（通用）→ 再宿主内嵌（专用）**。内核依然同时编译到 `wasm32`，内嵌那条路留着。

> ⚠️ 一个物理事实：**浏览器里的 WASM 壳不能当 MCP server**（没有进程、没有 stdio、没有入站连接）。
> 要经 MCP 被调用，就必须有一个 native 进程——这正是 `crates/native` 存在的理由之一。

## 2. 为什么是 MCP

因为它是**现在通用工具真正在读的那个接口**：VS Code（Copilot Chat / Agent 模式）、Cursor、Claude Code 都能连 MCP 服务，
而它只需要一个能读写 stdio 的进程——恰好是上面那条约束下唯一可行、又最不挑宿主的形态。
